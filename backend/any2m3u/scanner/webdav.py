...
class WebDAVAdapter:
    ...
    async def open_range(self, path: str, start: int, end: int | None) -> AsyncIterator[bytes]:
        """Stream bytes [start:end+1] from a file.

        If the upstream rejects Range (405/406/501), this method transparently
        retries without the Range header and yields the full body. The caller
        can detect the fallback by comparing the yielded length to the
        requested range, or by inspecting upstream semantics. Use open_full()
        if you specifically want a no-Range GET.
        """
        from .base import UpstreamRangeNotSupported
        client = await self._ensure_client()
        ...
        raise UpstreamRangeNotSupported(f"upstream does not support Range for {path}")
